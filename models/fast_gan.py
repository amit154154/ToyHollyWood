% % writefile / opt / conda / lib / python3
.7 / site - packages / lightweight_gan / lightweight_gan.py
...
class Trainer():
    ...
    def set_data_src(self, folder):
        ...
        self.dataset = ImageDataset(folder, self.image_size, transparent=self.transparent, greyscale=self.greyscale,
    ...
    def train(self):
        ...
        for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[D_aug, G]):
            ...
            if apply_gradient_penalty:
                ...
                scaled_gradients = torch_grad(outputs=outputs, inputs=image_batch,
                                              grad_outputs=list(
                                                  map(lambda t: torch.ones(t.size(), device=image_batch.device),
                                                      outputs)),
        ...
        for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[G, D_aug]):
            ...
            total_gen_loss += loss
        if self.steps % 10 == 0:
            wandb.log({'disc_loss':disc_loss,'gen_loss':gen_loss})
        if self.steps % 50 == 0:
            wandb.log({"samples": wandb.Image(generated_images[0:3])})
            ...
            for idx, images in enumerate(grouped):
                ...
                self.run.track(value=aim_images, name='generated',
                               ...
                               context={'interpolated': True,
                                        'alpha': alpha})
        torchvision.utils.save_image(generate_interpolations,
                                     str(self.results_dir / self.name / f'{str(num)}-interp.{ext}'), nrow=num_rows)
        # regular
