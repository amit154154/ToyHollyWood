...
class EMA:
    ...
    def update_average(self, old, new):
        return new if not exists(old) else old * self.beta + (1 - self.beta) * new
...
class RandomApply(nn.Module):
    ...
    def forward(self, x):
        fn = self.fn if random.uniform(0, 1) < self.prob else self.fn_else
        return fn(x)
...
def is_empty(t):
    return t.nelement() == 0 if isinstance(t, torch.Tensor) else not exists(t)
...
class Dataset(data.Dataset):
    def __init__(self,
        ...
        assert self.paths, f'No images were found in {folder} for training'

        convert_image_fn = convert_rgb_to_transparent if transparent else convert_transparent_to_rgb

        num_channels = 4 if transparent else 3
...
def random_hflip(tensor, prob):
    return tensor if prob <random.uniform(0, 1) else torch.flip(tensor, dims=(3,))
...
class Conv2DMod(nn.Module):
    ...
    @staticmethod
    def _get_same_padding(size, kernel, dilation, stride):
...
class Discriminator(nn.Module):
    def __init__(self, image_size, network_capacity=16, fq_layers=[], fq_dict_size=256, attn_layers=[],
        ...
        num_init_filters = 4 if transparent else 3
...
class Trainer():
    ...
    def load_config(self):
        config = json.loads(self.config_path.read_text()) if self.config_path.exists() else self.config()
    ...
    def set_data_src(self, folder):
        ...
        num_workers = default(self.num_workers, 0 if self.is_ddp else NUM_CORES)
    ...
    def train(self):
        ...
        if exists(self.GAN.D_cl):
            ...
            if apply_cl_reg_to_generated:
                for i in range(self.gradient_accumulate_every):
                    get_latents_fn = mixed_list if random.uniform(0, 1) < self.mixed_prob else noise_list
        ...
        for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[D_aug, S, G]):
            ...
            if self.steps % 50 == 0:
                wandb.log({'samples': wandb.Image(generated_images)})
        ...
        self.steps += 1
        wandb.log({'G': self.g_loss,
                   'D': self.d_loss}, step=self.steps)
        self.av = None
    ...
    def generate_interpolation(self, num=0, num_image_tiles=8, trunc=1.0, num_steps=100, save_frames=False):
        ...
        for ratio in tqdm(ratios):
            ...
            wandb.log({'image_grid': wandb.Image(pil_image)})
